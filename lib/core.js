...
const options = require("./middleware/fileops");
 ...
 *
 * @param {String}inputfile 一个用于作为输入的JSON文件路径
 */
exports.dataView = function (inputfile) {
    //定义输出文件
 ...
 *
 * @param {String}inputfile 一个用于作为输入的文件路径，文件的内容需要满足模版。
 */
exports.formData = function (inputfile) {
    //定义输出文件
    let outputfile = options.replaceExt(inputfile, ".json"),
        subjects = [];
    //读取题目和相关处理
    fs.readFile(inputfile, (error, data) => {
        if (error) {
...
/**合并两个JSON数据
 *
 * @param {String[]}source 一系列的JSON文件的路径
 * @return {Object[]}
 * @todo 过滤掉相同的内容
 */
